# src/train_hybrid.py
# Hybrid CNN-Transformer training script for RFMiD retinal fundus multi-disease classification
# - CoAtNet-0, CMT-Tiny (hybrid CNN/Transformer architectures)
...
def build_model(model_name, num_classes):
    """Return hybrid CNN-Transformer backbone for RFMiD classification (same head style for all)."""
    model_name = model_name.lower()
    ...
    # Add consistent classifier head for all models
    ...
    return ModelWrapper(backbone, classifier)
...
def run_for_model(model_name: str):
    ...
    # Optimizer/scheduler for hybrid models (AdamW + cosine with warmup)
    ...
    val_stats_npz = RESULTS_DIR / "hybrid_anyabnormal_val_outputs.npz"
    ...
    test_stats_npz = RESULTS_DIR / "hybrid_anyabnormal_test_outputs.npz"
...
# ----------------------
def generate_all_hybrid_overall_metrics():
    """Generate overall metrics for all hybrid models after training"""
    print("🚀 Generating Overall Test Results for All Hybrid Models")
    print("=" * 70)
    
    try:
        # Set up paths
        ROOT_DIR = Path(__file__).resolve().parent.parent
        RESULTS_DIR = ROOT_DIR / "results" / "Hybrid"
        DATA_DIR = ROOT_DIR / "data" / "RFMiD_Challenge_Dataset"
        ...
        for model_display_name, config in model_cfgs.items():
            ...
            # Load temperature if it exists
            temp_file = model_path / "temperature.txt"
            if temp_file.exists():
                T_eval = float(open(temp_file).read().strip())
                model = _WithTemp(model, T_eval).to(device)
                print(f"   ✅ Using temperature T = {T_eval:.3f}")
            else:
                print(f"   ⚠️ No temperature file found; using unscaled logits.")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--generate-metrics":
        # Generate overall metrics for all trained hybrid models
        success = generate_all_hybrid_overall_metrics()
        if success:
            print("\n✅ SUCCESS!")
            print("All hybrid models now have complete overall test metrics.")
        else:
            print("\n❌ FAILED!")
